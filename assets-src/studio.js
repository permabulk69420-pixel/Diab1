...
const ASSETS = {barrel: {build: buildBarrel, focus: [0, .46, 0], size: 1, close: [[.52, .98, .36], [.2, .8, .12]]}};
...
dungeon.add(new THREE.HemisphereLight(0x55607a, 0x1a120c, 1.1), floor(0x4a4540, .95), asset);
...
studio.add(new THREE.HemisphereLight(0xdfe8ff, 0x3a332c, .9), key, floor(0x77726c, .9), asset.clone());
const camB = new THREE.PerspectiveCamera(35, 1, .05, 50); camB.position.set(1.55, 1.55, 1.75); camB.lookAt(...A.focus);
const camC = new THREE.PerspectiveCamera(35, 1, .01, 50); camC.position.set(...A.close[0]); camC.lookAt(...A.close[1]);

renderer.setScissorTest(true);
[[dungeon, camA], [studio, camB], [studio, camC]].forEach(([s, c], i) => { renderer.setViewport(i * VW, 0, VW, VH); renderer.setScissor(i * VW, 0, VW, VH); renderer.render(s, c); });
...
window.__stats = {tris, meshes: asset.children.length};
window.__exportGLB = async () => {
  const buf = await new GLTFExporter().parseAsync(asset, {binary: true});
