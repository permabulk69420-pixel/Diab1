// Ported from Playground (magic-blast.js). Changes for Diab1: effects live in one group that
// follows the player between town and dungeon scenes, impacts use the area's collision via
// hitTest(from, dir, far) instead of raycasting meshes, and the tuning panel/target are gone.
...
export function createMagicBlast({ scene: initialScene, renderer, camera, rig, hands, hitTest, onHit }) {
  ...
  /** Move the effects into the scene the player is now in (town <-> dungeon). */
  function setScene(next) { if (scene.parent !== next) { next.add(scene); reset(); } }
  /** Desktop preview: drives the same recognizer with simulated hand motion. */
  function preview() { if (!renderer.xr.isPresenting) { reset(); demo = true; demoTime = 0; } }

  return { update, reset, setScene, preview, gesture, get stats() { return { shots, hits }; } };
}
